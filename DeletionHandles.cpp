...
void deleteImages(sql::Connection* con, std::size_t threadId, std::size_t commentId){
	...
	if(threadId != -1){
		prepStmt = std::unique_ptr<sql::PreparedStatement>(con->prepareStatement("SELECT id FROM images WHERE threadId = ?"));
		prepStmt->setInt64(1, threadId);
	}
	else if(commentId != -1){
		prepStmt = std::unique_ptr<sql::PreparedStatement>(con->prepareStatement("SELECT id FROM images WHERE commentId = ?"));
		prepStmt->setInt64(1, commentId);
	}
	std::unique_ptr<sql::ResultSet> res(prepStmt->executeQuery());
	res->beforeFirst();
	while(res->next()){
		deleteImage(con, res->getInt64("id"));
	}
}
