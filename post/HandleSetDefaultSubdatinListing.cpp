...
void handleSetDefaultSubdatinListing(FcgiData* fcgi, std::vector<std::string> parameters, void* _data){
	...
	default:
		createGenericErrorPage(fcgi, data, "Invalid List Number");
		return;
	...
	default:
		createGenericErrorPage(fcgi, data, "Invalid Subdatin Title");
		return;
	case InputError::NoError:
		break;
	}
	...
	if(subdatinId == -1){
		createGenericErrorPage(fcgi, data, "That Subdatin Does Not Exist");
		return;
	}
