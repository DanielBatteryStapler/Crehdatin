#include "HandleSetCurator.h"

void handleSetCurator(FcgiData* fcgi, std::vector<std::string> parameters, void* _data){
	...
	std::unique_ptr<sql::PreparedStatement>prepStmt(data->con->prepareStatement("UPDATE userPositions SET userPosition = 'curator' WHERE userId = ? AND subdatinId = ?"));
