...
void handleSetSubdatinCommentLocked(FcgiData* fcgi, std::vector<std::string> parameters, void* _data){
	...
	if(getPostValue(fcgi->cgi, authToken, "authToken", Config::getUniqueTokenLength(), InputFlag::AllowStrictOnly) != InputError::NoError 
		...
		createSetSubdatinCommentLockedErrorPage(fcgi, data, -1, "Invalid Authentication Token");
		return;
	}
	...
	if(subdatinId == -1){
		createSetSubdatinCommentLockedErrorPage(fcgi, data, -1, "Cannot edit a subdatin that doesn't exist");
	}
	else{
		if(!hasSubdatinControlPermissions(getEffectiveUserPosition(data->con, data->userId, subdatinId))){
			createSetSubdatinCommentLockedErrorPage(fcgi, data, subdatinId, "You do not have the correct permissions to do this");
			return;
		}
		
		std::string locked;
		if(getPostValue(fcgi->cgi, locked, "locked", 10, InputFlag::AllowStrictOnly) != InputError::NoError){
			createSetSubdatinCommentLockedErrorPage(fcgi, data, subdatinId, "Invalid 'Locked' Valid");
			return;
		}
...
void createSetSubdatinCommentLockedErrorPage(FcgiData* fcgi, RequestData* data, int64_t subdatinId, std::string error){
